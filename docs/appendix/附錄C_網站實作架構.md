...
1. **localStorage 範圍**：資料存於使用者瀏覽器，清除瀏覽器資料即遺失；正式環境建議改接後端。
   系統定位（單機試算工具）、個資/資安合規差距分析與後端演進路線，見 `docs/compliance/`（入口：[合規00 系統定位與適用邊界聲明](../compliance/合規00_系統定位與適用邊界聲明.md)、[合規01 主報告](../compliance/合規01_合規差距分析主報告.md)）。
   1.12 起的資安層（`src/lib/security/`＋`components/security/AppGate|LockScreen`）：可選**密碼鎖**
   （AES-256-GCM 本機加密，未啟用＝明文 passthrough 零回歸；忘記密碼不可救）、**稽核雜湊鏈**
   （tamper-evident，非防竄改）、**加密備份**、build 時注入 CSP。邊界與殘餘風險以合規00 §7 為準。
