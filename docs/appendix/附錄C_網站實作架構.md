...
| 專案成本 | `ReportsView`／`AnalyticsView`／`SettingsView`／`MonthlyView` | 專案主檔（`SettingsView` CRUD）＋每月工時分攤（`AllocationEditor`，時數/百分比）；`lib/reports/projectCost.ts` 以 `distributeRounded` 把每員工全載成本依比例切到各專案＋未分攤桶（勾稽：Σ＝全公司總成本），算 FTE/稼動率/預算 vs 實際；報表頁專案 P&L＋分析頁「專案成本」分頁（StackedBar/Pareto/Bullet＋`analyzeProjectCost` 白話意見） |
