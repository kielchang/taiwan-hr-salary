...
| 每月作業③ | `ReportsView` 報表與薪資條 | 三維度匯總報表＋本月代扣所得稅清單（建議值/已填值分離、可帶入）＋逐人薪資條（§23）皆可列印 |
| 說明 | `HelpView` 使用說明／`SourcesView` 法規與費率依據 | 純操作說明＋FAQ；法源出處卡片（法條＋主管機關官網連結），即來源01–05 的使用者版 |
