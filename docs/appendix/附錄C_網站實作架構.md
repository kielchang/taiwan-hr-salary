...
| 說明 | `HelpView` 使用說明／`WikiView` 操作手冊（/wiki，8 章完整手冊：內容註冊表 `src/content/wiki/`＋搜尋/列印/深連結/導覽串接）／`SourcesView` 法規與費率依據 | 純操作說明＋FAQ＋薪酬分析指引；法源出處卡片（法條連結指向全國法規資料庫條文＋主管機關），即來源01–05 的使用者版 |
