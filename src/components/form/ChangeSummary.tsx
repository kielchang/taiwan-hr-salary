      ...
      <ul className={cn("space-y-1", changes.length > 8 && "max-h-56 overflow-y-auto pr-1")}>
        {changes.map((c) => (
          <li key={c.field} className="flex items-center gap-2 text-xs">
            <Tooltip content={c.label} className="w-24 shrink-0"><span className="block truncate text-muted-foreground">{c.label}</span></Tooltip>
            <Tooltip content={c.beforeText} className="min-w-0"><span className="block truncate text-muted-foreground line-through">{c.beforeText}</span></Tooltip>
            <ArrowRight className="size-3 shrink-0 text-amber-500" />
            <Tooltip content={c.afterText} className="min-w-0"><span className="block truncate font-medium text-amber-900">{c.afterText}</span></Tooltip>
            {onRevertField && (
